...
void UserGUI::HandleMenu(float width, float height, 
	...
	ImGui::SetWindowPos(ImVec2(screenWidth - width, 0));
    ...
    if (ImGui::BeginTabBar("Generation Items")) {
        ...
        if (ImGui::BeginTabItem("Branches")) {
            if (ImGui::ColorPicker4("Base branch color", baseBranchColor)); ImGui::NewLine();

            ImGui::EndTabItem();
        }

        if (ImGui::BeginTabItem("Leaves")) {
            if (ImGui::ColorPicker4("Base leaf color", baseLeafColor)); ImGui::NewLine();
